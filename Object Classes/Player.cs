    ...
    public class Player {
        ...
        public void Play(Die d1, Die d2)
        {
            ...
            if (hasPower == true)
            {
                d1.Roll();
                d2.Roll();
                int totalStep = d1.FaceValue + d2.FaceValue;
                position += totalStep;
                //Board board;
                // ACT 1 : Move player with the amount of steps drawn by d1 and d2
                if (ReachedFinalSquare() == true)   // MAKE USE OF THE ReachFinalSquare
                {
